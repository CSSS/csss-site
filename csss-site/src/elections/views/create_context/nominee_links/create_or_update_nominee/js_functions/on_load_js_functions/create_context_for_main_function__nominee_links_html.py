from elections.views.Constants import NOMINEE_DIV__NAME, DRAFT_NOMINEE_HTML__NAME, NA_STRING, NOMINEE_LINK_SFU_ID
...
def create_context_for_main_function__nominee_links_html(
        context, nominee_info=None, nominee_obj=None, election_officer_request=True, sfuid=""):
    """
    ...
    SFU ID -- the sfu ID attached to the Nominee Link
    """
    context.update({
    ...
    if nominee_info is not None or nominee_obj is not None:
        if nominee_info is not None:
            ...
            for (key, value) in nominee_info_to_add_to_context.items():
                # iterating over the values to ensure any None values created above get converted to NA instead
                if value is None:
                    nominee_info_to_add_to_context[key] = NA_STRING
            create_context_for_display_nominee_info_html(
                ...
                populate_nominee_info=True, election_officer_request=election_officer_request
            )
        ...
        if election_officer_request:
            context[NOMINEE_LINK_SFU_ID] = sfuid
        create_context_for_display_nominee_info_html(
