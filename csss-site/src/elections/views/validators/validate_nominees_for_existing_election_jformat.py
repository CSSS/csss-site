...
from elections.views.ElectionModelConstants import ELECTION_JSON_KEY__NOM_NAME, ELECTION_JSON_KEY__NOM_FACEBOOK, \
    ELECTION_JSON_KEY__NOM_EMAIL, ELECTION_JSON_KEY__NOM_LINKEDIN, \
...
def validate_nominees_for_existing_election_jformat(election_id, nominees):
    ...
    for nominee in nominees:
        if not (ELECTION_JSON_KEY__NOM_NAME in nominee and ELECTION_JSON_KEY__NOM_SFUID in nominee
                and ELECTION_JSON_KEY__NOM_FACEBOOK in nominee
                ...
                and ELECTION_JSON_KEY__NOM_DISCORD_ID in nominee
            ...
            return False, f"It seems that one of the nominees is missing one of the following fields:" \
                          f" {ELECTION_JSON_KEY__NOM_NAME}, {ELECTION_JSON_KEY__NOM_SFUID}, " \
                          f"{ELECTION_JSON_KEY__NOM_FACEBOOK}, " \
                          f"{ELECTION_JSON_KEY__NOM_INSTAGRAM}, " \
                          f"{ELECTION_JSON_KEY__NOM_LINKEDIN}, " \
                          f"{ELECTION_JSON_KEY__NOM_EMAIL}, {ELECTION_JSON_KEY__NOM_DISCORD_ID}, " \
        ...
        success, error_message = validate_existing_nominee_jformat(
            ...
            nominee[ELECTION_JSON_KEY__NOM_NAME], nominee[ELECTION_JSON_KEY__NOM_SFUID],
            ...
            nominee[ELECTION_JSON_KEY__NOM_EMAIL], nominee[ELECTION_JSON_KEY__NOM_DISCORD_ID],
            election_id)
        if not success:
