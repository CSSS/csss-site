...
from elections.views.election_management import NOM_NAME_POST_KEY, \
    NOM_POSITIONS_POST_KEY, NOM_SPEECH_POST_KEY, NOM_FACEBOOK_POST_KEY, NOM_LINKEDIN_POST_KEY, NOM_EMAIL_POST_KEY, \
    NOM_DISCORD_USERNAME_POST_KEY, NOM_POSITION_AND_SPEECH_POST_KEY, NOM_ID_POST_KEY, NOM_POSITION_POST_KEY
...
def _validate_existing_nominee(full_name, position_names_and_speech_pairing, facebook_link, linkedin_link,
    ...
    for position_names_and_speech_pairing in position_names_and_speech_pairing:
        ...
        success, error_message, specified_position_names = position_in_pairing(
...
def position_in_pairing(speech_and_position_pairing, specified_position_names, election_id, name):
    ...
    if not there_are_multiple_entries(speech_and_position_pairing, NOM_POSITIONS_POST_KEY):
        return False, f"It seems that the nominee {name} does not have a list of positions " \
                      f"for one of the positions they are running for", specified_position_names
    for position in speech_and_position_pairing[NOM_POSITIONS_POST_KEY]:
        if NOM_ID_POST_KEY in position:
            if f"{position[NOM_ID_POST_KEY]}".isdigit() and NomineePosition.objects.all().filter(
                    id=int(position[NOM_ID_POST_KEY]), nominee_speech__nominee__election_id=election_id
            ):
                return True, None, specified_position_names
            else:
                return False, \
                       f"One of the IDs specified for one of the positions that {name} is running for is invalid", \
                       specified_position_names
        if not (NOM_POSITION_POST_KEY in speech_and_position_pairing and
                len(speech_and_position_pairing[NOM_POSITION_POST_KEY]) > 0):
            return False, f"One of the position names specified {name} is invalid", specified_position_names
        position_name = speech_and_position_pairing[NOM_POSITION_POST_KEY]
        if len(OfficerEmailListAndPositionMapping.objects.all().filter(position_name=position_name)) == 0:
