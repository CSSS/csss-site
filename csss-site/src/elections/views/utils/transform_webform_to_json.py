...
def transform_full_election_webform_to_json(election_dict):
    """
    ...
    """
    logger.info("[elections/transform_webform_to_json.py transform_full_election_webform_to_json()] transforming")
    ...
    if ELECTION_JSON_KEY__NOMINEES in election_dict and type(election_dict[ELECTION_JSON_KEY__NOMINEES]) == dict:
        ...
        for nominee in new_election_dict[ELECTION_JSON_KEY__NOMINEES]:
            _transform_nominee_webform_to_json(nominee)
    ...
    logger.info("[elections/transform_webform_to_json.py transform_full_election_webform_to_json()] to")
    logger.info(json.dumps(new_election_dict, indent=3))
    return new_election_dict


def transform_nominee_links_webform_to_json(nominee_dict):
    """
