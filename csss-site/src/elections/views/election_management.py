...
from csss.views_helper import verify_access_logged_user_and_create_context, \
    there_are_multiple_entries, ERROR_MESSAGE_KEY
...
def create_specified_election(request):
    logger.info(f"[administration/list_nominees.py create_specified_election()] request.POST={request.POST}")
    ...
    if ELECTION_TYPE_KEY in request.POST and ELECTION_DATE_POST_KEY in request.POST and \
        ...
        logger.info("[administration/list_nominees.py create_specified_election()] creating new election")
        nomination_page = get_nomination_page(request.POST)
        post_dict = parser.parse(request.POST.urlencode())
        logger.info(f"[administration/list_nominees.py create_specified_election()] post_dict={post_dict}")
        logger.info(
            "[administration/list_nominees.py create_specified_election()] "
...
def save_nominees(post_dict, nomination_page, position_index):
    for i in range(len(post_dict[NOM_NAME_KEY])):
        ...
        logger.info(
            "[administration/list_nominees.py save_nominees()] saved user "
        ...
        if full_name != 'NONE':
            ...
            logger.info(
                "[administration/list_nominees.py save_nominees()] skipping saving user "
                f"full_name={full_name} officer_position={officer_position} speech={speech} "
                f"facebook_link={facebook_link} linkedin_link={linkedin_link} "
                f"email_address={email_address}  discord_username={discord_username}"
            )
...
def create_or_update_specified_election_with_provided_json(request):
    logger.info(
        "[administration/list_nominees.py create_or_update_specified_election_with_provided_json()] "
...
def save_nominees_from_json(nominees, nomination_page):
    ...
    for nominee in nominees:
        ...
        logger.info(
            "[administration/list_nominees.py save_nominees_from_json()] saved user "
            f"full_name={full_name} officer_position={officer_position} speech={speech} "
...
def select_election_to_update(request):
    ...
    return render(request, 'elections/select_election.html', context)
...
def determine_election_action(request):
    logger.info(f"[administration/list_nominees.py determine_election_action()] request.POST={request.POST}")
    (render_value, error_message, context) = verify_access_logged_user_and_create_context(request, TAB_STRING)
    if context is None:
        request.session[ERROR_MESSAGE_KEY] = '{}<br>'.format(error_message)
        return render_value
    if 'action' in request.POST:
        if request.POST['action'] == DELETE_ACTION_POST_KEY and ELECTION_ID_KEY in request.POST:
            ...
            logger.info(
                "[administration/list_nominees.py determine_election_action()] incorrect action detected, "
                "returning /elections/select_election"
            )
            return HttpResponseRedirect(f"{settings.URL_ROOT}elections/select_election")
    else:
        logger.info(
            "[administration/list_nominees.py determine_election_action()] action "
...
def display_selected_election_for_updating(request, election_id):
    ...
    return render(request, 'elections/update_election.html', context)
...
def update_specified_election(request):
    ...
    if ELECTION_TYPE_KEY in request.POST and ELECTION_DATE_POST_KEY in request.POST \
        ...
        if len(post_dict[NOM_NAME_KEY][0]) > 1:
            ...
            if full_name != 'NONE':
                logger.info(
                    "[administration/list_nominees.py update_specified_election()] "
                    f"saved user full_name={full_name} officer_position={officer_position} "
