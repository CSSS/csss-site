            ...
            for updated_cron_job in updated_cron_jobs:
                ...
                if job is None:
                    ...
                    job = scheduler.add_job(
                        ...
                        trigger=CronTrigger.from_crontab(updated_cron_job.schedule, timezone=TIME_ZONE)
                    )
                    previous_job_id = updated_cron_job.job_id
                    updated_cron_job.job_id = job.id
                    updated_cron_job.save()
                    logger.error(
                        "[Cron_Service_Command handle()] unable to find a cron job id of "
                        f"{previous_job_id} for {updated_cron_job.job_name}."
                        f"A new cron job was created as a result with id of {updated_cron_job.job_id}"
                    )
                else:
                    if updated_cron_job.schedule == "":
                        logger.info(
                            f"[Cron_Service_Command handle()] removing cron job {updated_cron_job.job_name}"
                            f" since the schedule is now empty "
                        )
                        ...
                        logger.info(
                            ...
                            f"with schedule of {job.trigger} to {updated_cron_job.schedule}"
                        )
                        script_path = CRON_JOB_MAPPING[updated_cron_job.job_name][CRON_JOB_MAPPING_PATH_KEY]
