...
class GoogleDrive:
    ...
    def add_users_gdrive(self, users, file_id=None):
        ...
        for user in users:
            emailMessage=(
                f"Hello {user},"
                "You have been granted acces to a subfolder in the CSSS Google Drive. Please be careful when deleting as you have \"master\" access, as do all the execs."
                "Furthermore, when creating a file on the CSSS Google Drive, please try to transfer ownerships of any files you create to \"sfucsss@gmail.com\" account "
                "as this makes it easier for me to remove your access when it's time to revoke it."
                "You can figure out how to do that with these instructions: https://github.com/CSSS/managingCSSSResources"
                "-Your Sys Admin"
            )
            body={}
            body['role']='writer'
            ...
            try:
                print(f"[GoogleDrive add_users_gdrive()] attempting to give {user.lower()} permission to access the sfu csss google drive")
                self.gdrive.permissions().create(fileId=file_id,emailMessage=emailMessage,sendNotificationEmail=True, body=body).execute()
                print(f"[GoogleDrive add_users_gdrive()] email sent to {user.lower()} regarding their access to the sfu google drive")
    ...
    def remove_users_gdrive(self, users, file_id=None):
        ...
        if file_id is None:
            file_id = self.root_file_id
        try:
            print(f"[GoogleDrive remove_users_gdrive()] attempting to get the list of permisisons for file with id {file_id}")
            permissions = self.gdrive.permissions().list(fileId=file_id,fields='permissions').execute()
            print(f"[GoogleDrive remove_users_gdrive()] was able to get the list of file permissions")
            for user in users:
                for permission in permissions['permissions']:
                    if permission['emailAddress'].lower() == user:
                        try:
                            print(f"[GoogleDrive remove_users_gdrive()] attempting to remove user {user}'s access to file with id {file_id}")
                            self.gdrive.permissions().delete(fileId = file_id, permissionId = permission['id']).execute()
                            print(f"[GoogleDrive remove_users_gdrive()] attempt successful")
                        except Exception as e:
                            print(f"[GoogleDrive remove_users_gdrive()] encountered following error with permission removed. \n {e}")
        except Exception as e:
            print(f"[GoogleDrive remove_users_gdrive()] encountred the following error when trying to get the list of permissions. \n{e}")

    def make_public_link_gdrive(self, file_id, file_name):
        ...
        file_name -- the name of the file, needed to make the file information human-readable
        """
        if file_id is None or file_name is None:
            print("[GoogleDrive make_public_link_gdrive()] Please specify a valid file_id and file_name")
            return
        body = {}
        body['role'] = 'writer'
        body['type'] = 'anyone'
        try:
            print(f"[GoogleDrive make_public_link_gdrive()] will attempt to make the file with id {file_id} publicly available." )
            response = self.gdrive.permissions().create(fileId=file_id, body=body).execute()
            print(f"[GoogleDrive make_public_link_gdrive()] will attempt to get the public link to file.")
            response = self.gdrive.files().get(fileId=file_id, fields='webViewLink').execute()
            file = GDrivePublicFiles(
                file_id = file_id,
            ...
            print(f"[GoogleDrive make_public_link_gdrive()] encountered the following error. \n {e}")
    ...
    def remove_public_link_gdrive(self, file_id):
        ...
        try:
            ...
            print(f"[GoogleDrive remove_public_link_gdrive()] removed public link for file with id {file_id}")
        except Exception as e:
            print(
    ...
    def check_parent_folder(self, file_id, parent_folder):
        ...
        """
        while True:
            daResponse = self.gdrive.files().get(fileId=file_id, fields='*').execute()
            # print(json.dumps(daResponse, indent=4, sort_keys=True))
            if daResponse['id'] == parent_folder:
                return True
            if 'parents' not in daResponse:
                return False
            file_id = daResponse['parents'][0]
            # id, name and parent id

    def ensure_root_permissions_are_correct(self, google_drive_perms):
        """Attempts to make sure that only execs [and other necessary individuals] have access to the CSSS root folder
        ...
        try:
            ...
            print(f"[GoogleDrive ensure_root_permissions_are_correct()] unable to get all the files under root")
            return
        ...
        # ensure_permissions_are_correct to deal with cases where subfiles/folders are now owned by sfucsss@gmail.com
        # which prevents removing the owner of that file from the drive
        time.sleep(10)
    ...
    def ensure_permissions_are_correct(self, google_drive_perms, parent_id):
        """Goes through each single file and folder under "CSSS" root folder and checking each file/folder to make sure
         that either it has the proper permission sets or is duplicated or its owner notified that they need to correct
         who the owner of the file is so that its permissions can be corrected
        ...
        while True:
            try:
                ...
                print(
                    ...
                    f"under folder with id {parent_id[len(parent_id)-1]}"
                )
                return
                ...
                for permission in file['permissions']:
                    ...
                    # check files that are link-share enabled
                        if 'anyoneWithLink' not in google_drive_perms.keys():
                            ...
                            print(
                                f"[GoogleDrive ensure_permissions_are_correct()] removing public "
                                "link for file with id {file['id']} and name {file['name']}"
                            )
    ...
    def ensure_correct_owner_for_file(self, file_info):
        ...
        if 'google-apps' not in file_info['mimeType']:
            # any file that is not a google native document cannot have its owner changes, it needs to be duplicated instead.
    ...
    def duplicate_file(self, file_info):
        ...
        try:
            ...
            try:
                response = self.gdrive.files().update(fileId=file_info['id'], body=body).execute()
            except Exception as e:
                print(f"[GoogleDrive duplicate_file()] counldnt update {file_info['name']} to \"duplicated_and_removed\"")
                return

            # google drive api doesnt allow a function that "removes" a file from the sfucsss@gmail.com's google drive.
            ...
            for permission in file_info['permissions']:
                if "emailAddress" in permission:
                    if permission['emailAddress'].lower() == "sfucsss@gmail.com":
                        ...
                        # keep its access to the file in order to remove any remaining non-owner permissions that may exist.
                        sfucsss_permission_id = permission['id']
                    else:
                        try:
                            response = self.gdrive.permissions().delete(fileId=file_info['id'], permissionId=permission['id']).execute()
                            print(f"[GoogleDrive duplicate_file()] removed {permission['emailAddress'].lower()}'s access to {file_info['name']}")
                        except Exception as e:
                            print(f"[GoogleDrive duplicate_file()] couldnt remove {permission['emailAddress'].lower()}'s access to {file_info['name']}")
                            return
            response = self.gdrive.permissions().delete(fileId=file_info['id'], permissionId=sfucsss_permission_id).execute()
        except Exception as e:
            print(f"[GoogleDrive duplicate_file()] unable to duplicate and remove file from drive")


    def alert_user_to_change_owner(self, file_info):
        try:
            body = {}
            body['content'] = "Please change owner of this file to sfucsss@gmail.com.\nInstructions for doing so can be found here: https://github.com/CSSS/managingCSSSResources"
            resp = self.gdrive.comments().create(fileId=file_info['id'], fields="*", body=body).execute()
            print(f"[GoogleDrive alert_user_to_change_owner()] comment added to file {file_info['name']}")
        except Exception as e:
            print(f"[GoogleDrive alert_user_to_change_owner()] unable to add comment to file {file_info['name']}")
