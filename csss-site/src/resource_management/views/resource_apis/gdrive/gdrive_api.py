...
class GoogleDrive:
    ...
    def _file_is_gdrive_form(self, file_info):
        ...
        if 'mimeType' in file_info:
            self.logger.info(
                f"[GoogleDrive _file_is_gdrive_form()] file type for file {file_info['name']} "
                f"is {file_info['mimeType']}"
            )
        else:
            self.logger.error(
                "[GoogleDrive _file_is_gdrive_form()] "
                f"unable to find key 'mimeType' for file {file_info['name']}"
            )
        return file_type
    ...
    def _alert_user_to_change_owner(self, file_info):
        ...
        try:
            ...
            self.logger.info(
                f"[GoogleDrive _alert_user_to_change_owner()] attempting to add a comment added "
            ...
            if self.make_changes:
                ...
                if response['content'] == body['content']:
                    ...
                    self.logger.error(
                        f"[GoogleDrive _alert_user_to_change_owner()] unable to add comment"
                        f" to file {file_info['name']}"
                    )
        except Exception as e:
            self.logger.error(
    ...
    def _remove_outdated_comments(self, file_info):
        ...
        try:
            self.logger.info(
                f"[GoogleDrive _remove_outdated_comments()] attempting to remove any comments that "
