...
def process_specified_new_officers(request, context, saved_unprocessed_officers, officer_emaillist_and_position_mappings):
    """
    ...
    if not (id_of_new_officer_to_send_link_to is not None or save_or_update_new_officers_option_selected):
        ...
        create_context_for_specify_new_officers_html(
            context, saved_unprocessed_officers, officer_emaillist_and_position_mappings, error_messages=[error_message],
    ...
    if len(unprocessed_officers) == 0:
        ...
        success, error_message = validate_inputted_unprocessed_officers(
            saved_unprocessed_officers, officer_emaillist_and_position_mappings, officers, terms, inputted_term, inputted_year, unprocessed_officers=unprocessed_officers
        )
        if not success:
            logger.info(
                f"[about/process_specified_new_officers.py process_specified_new_officers()] {error_message}"
        ...
        success, error_message = save_unprocessed_officers(
            saved_unprocessed_officers, officer_emaillist_and_position_mappings, officers, terms, inputted_term, inputted_year, unprocessed_officers
        )
        if not success:
            logger.info(
                f"[about/process_specified_new_officers.py process_specified_new_officers()] {error_message}"
