...
def transform_webform_to_json(new_officers_dict):
    ...
    Return
    unprocessed_officers -- the list of unprocessed officers the user inputted
    ...
    if NEW_OFFICERS__HTML_VALUE in new_officers_dict and type(new_officers_dict[NEW_OFFICERS__HTML_VALUE]) == dict:
        unprocessed_officers = list(new_officers_dict[NEW_OFFICERS__HTML_VALUE].values())
        for new_officer in unprocessed_officers:
            if ID_KEY in new_officer and len(new_officer[ID_KEY]) == 0:
                del new_officer[ID_KEY]
    inputted_term = new_officers_dict.get(TERM_KEY, None)
    inputted_year = new_officers_dict.get(YEAR_KEY, None)
    ...
    return (
        unprocessed_officers, inputted_term, inputted_year, id_of_new_officer_to_send_link_to,
