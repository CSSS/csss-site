...
def create_context_for_specify_new_officers_html(
    ...
    context[INPUT_REDIRECT_SAVE_OR_UPDATE_NEW_OFFICERS_SUBMIT_AND_CONTINUE_EDITING_VALUE] = SAVE_OR_CREATE_ALL_NEW_OFFICERS_INFORMATION
    context[SAVED_NOMINEES_EXIST] = len(new_officers) > 0

    new_officer_infos_to_display = create_context_for_main_function_html(
        context, current_date=current_date, draft_new_officers=draft_new_officers
    )
