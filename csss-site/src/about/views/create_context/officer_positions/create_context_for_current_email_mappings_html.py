    ...
    for officer_emaillist_and_position_mapping in officer_emaillist_and_position_mappings:
        if officer_emaillist_and_position_mapping.email != "NA":
            ...
            if officer_emaillist_and_position_mapping.executive_officer:
                current_and_past_minus_1_executive_officers.extend(officers_to_show[1:3])
                current_executive_officers.extend(officers_to_show[:1])
            context[EMAIL_LIST_MAPPINGS].append({
                SFU_EMAIL_LIST: officer_emaillist_and_position_mapping.email.replace("-current", ""),
                SFU_COMPUTING_IDS: officers_to_show
            })
            context[EMAIL_LIST_MAPPINGS].append({
                SFU_EMAIL_LIST: officer_emaillist_and_position_mapping.email,
                SFU_COMPUTING_IDS: officers_to_show[:1]
            })
    current_executive_officers = list(set(current_executive_officers))
    current_and_past_minus_1_executive_officers = list(set(current_and_past_minus_1_executive_officers))
    context[EMAIL_LIST_MAPPINGS].append({
        SFU_EMAIL_LIST: CURRENT_EXEC_EMAIL_LIST,
        SFU_COMPUTING_IDS: current_executive_officers
    })
    context[EMAIL_LIST_MAPPINGS].append({
        SFU_EMAIL_LIST: CURRENT_AND_PAST_MINUS_1_EXEC_EMAIL_LIST,
        SFU_COMPUTING_IDS: current_and_past_minus_1_executive_officers
    })
