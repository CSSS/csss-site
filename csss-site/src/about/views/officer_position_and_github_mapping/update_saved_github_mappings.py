...
from about.views.position_mapping_helper import update_context, extract_valid_officers_indices_selected_for_github_team
...
def toggle_deletion_status_for_github_mapping(post_dict, delete):
    ...
    github_team.save()
    logger.info(
        f"[about/position_mapping_helper.py mark_for_deletion_github_mapping()] github_team {github_team.team_name} has been marked for deletion")
    return ERROR_MESSAGES
...
def update_github_mapping(post_dict):
    ...
    logger.info(
        f"[about/position_mapping_helper.py update_github_mapping()] officer_position_indices : {officer_position_indices}")
    ...
    githubAPI = GitHubAPI(settings.GITHUB_ACCESS_TOKEN)
    if github_team_db_obj.team_name != new_github_team_name:
        success, error_message = githubAPI.rename_team(github_team_db_obj.team_name, new_github_team_name)
        if not success:
    ...
    success, error_messages = revoke_officer_with_specified_indices_access_to_specified_github_team(github_team_db_obj,
                                                                                                    githubAPI, term_obj,
    ...
    success, error_messages = grant_officers_with_specified_indices_access_to_specified_github_team(github_team_db_obj,
                                                                                                    githubAPI, term_obj,
...
def get_indices_for_officer_positions_that_need_access_revoked(github_team_db_obj, officer_position_indices):
    """
    Returns a list of all the position_indices that correspond to the officers who need to have their access to a github team revoked
    ...
    Return
    position_indices_for_officers_who_need_to_have_access_to_github_team_revoked -- a list of all the position_indices that correspond to the officers who need to have their access to a github team revoked
    """
    officer_github_team_mappings_who_currently_have_access_to_github_team = [officer for officer in
                                                                             OfficerPositionGithubTeamMappingNew.objects.all().filter(
                                                                                 github_team=github_team_db_obj)]
    logger.info(
        f"[about/position_mapping_helper.py get_officers_that_need_to_have_their_github_access_updated()] officer_github_team_mappings_who_currently_have_access_to_github_team = {officer_github_team_mappings_who_currently_have_access_to_github_team}")

    position_indices_for_officers_who_need_to_have_access_to_github_team_revoked = []
    for current_officer_position_index_with_github_membership in officer_github_team_mappings_who_currently_have_access_to_github_team:
        if current_officer_position_index_with_github_membership.officer_position_mapping.position_index not in officer_position_indices:
            position_indices_for_officers_who_need_to_have_access_to_github_team_revoked.append(
                current_officer_position_index_with_github_membership.officer_position_mapping.position_index)
    logger.info(
        f"[about/position_mapping_helper.py get_officers_that_need_to_have_their_github_access_updated()] position_indices_for_officers_who_need_to_have_access_to_github_team_revoked = {position_indices_for_officers_who_need_to_have_access_to_github_team_revoked}")
    return position_indices_for_officers_who_need_to_have_access_to_github_team_revoked


def revoke_officer_with_specified_indices_access_to_specified_github_team(github_team_db_obj, githubAPI, term_obj,
                                                                          position_indices_for_officers_who_need_to_have_access_to_github_team_revoked):
    """
    ...
    Keyword Argument
    github_team -- the github team object
    githubAPI -- the API object for github
    term_obj -- the term object for the current term
    position_indices_for_officers_who_need_to_have_access_to_github_team_revoked -- list of all the position_indices that correspond to the officers who need to have their access to a github team revoked
    ...
    """
    error_messages = []
    for position_index in position_indices_for_officers_who_need_to_have_access_to_github_team_revoked:
        ...
        if len(officers) > 0:
            success, error_message = githubAPI.remove_users_from_a_team([officers[0].github_username],
                                                                        github_team_db_obj.team_name)
            if not success:
                error_messages.append(error_message)
        else:
            logger.info(
                f"[about/position_mapping_helper.py update_existing_github_team_mappings()] unable to find officer under term {term_obj} for position_index {position_index}")
...
def get_indices_for_officer_positions_that_need_access_granted(github_team_db_obj, officer_position_indices):
    """
    Returns a list of all the position_indices that correspond to the officers who need to have their access to a github team granted
    ...
    Return
    position_indices_for_officers_who_need_to_have_access_to_github_team_granted -- a list of all the position_indices that correspond to the officers who need to have their access to a github team granted
    """
    ...
    for officer_position_index in officer_position_indices:
        if officer_position_index not in position_indices_for_officers_who_currently_have_access_to_github_team:
            position_indices_for_officers_who_need_to_have_access_to_github_team_granted.append(officer_position_index)
    logger.info(
        f"[about/position_mapping_helper.py get_officers_that_need_to_have_their_github_access_updated()] position_indices_for_officers_who_need_to_have_access_to_github_team_granted = {position_indices_for_officers_who_need_to_have_access_to_github_team_granted}")

    return position_indices_for_officers_who_need_to_have_access_to_github_team_granted


def grant_officers_with_specified_indices_access_to_specified_github_team(github_team, githubAPI, term_obj,
                                                                          position_indices_for_officers_who_need_to_have_access_to_github_team_granted):
    """
    ...
    error_messages -- a list of possible error messages
    """
    for position_index_for_officer_who_need_to_have_access_to_github_team_granted in position_indices_for_officers_who_need_to_have_access_to_github_team_granted:
        ...
        if len(position_mapping) > 0:
            ...
            logger.info(
                f"[about/position_mapping_helper.py update_existing_github_team_mappings()] saving a mapping of {position_mapping} under team {github_team}")
    ...
    for position_index_for_officer_who_need_to_have_access_to_github_team_granted in position_indices_for_officers_who_need_to_have_access_to_github_team_granted:
        ...
        if len(officer) > 0:
            officer = officer[0]
            success, error_message = githubAPI.add_users_to_a_team([officer.github_username], github_team.team_name)
            if not success:
                error_messages.append(error_message)
        else:
            logger.info(
                f"[about/position_mapping_helper.py update_existing_github_team_mappings()] unable to find any officers for term {term_obj} and position_index {position_index_for_officer_who_need_to_have_access_to_github_team_granted}")
    if len(error_messages) == 0:
        return True, None
    return False, error_messages
